...
function prettifyError(error: $ZodError, formatObjectPath: (string: string) => string = String): string {
	const lines: string[] = [];
	// sort by path length
...
export function reportErrors(
	...
	const formattedErrors = errors.map(({ key, receivedValue, error, defaultUsed, defaultValue }) => {
		...
		const typeSchema = (schemas[key] && "schema" in schemas[key] ? schemas[key].schema : schemas[key]) as z.ZodType;
