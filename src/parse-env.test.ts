...
describe("parseCore", () => {
	...
	it("handles a positive int", () => {
		expect(parseEnv({ dogCount: "34" }, { dogCount: z.number() })).toStrictEqual({
			dogCount: 34,
		});
	});

	it("handles a positive float", () => {
		expect(parseEnv({ dogCount: "29.453" }, { dogCount: z.number() })).toStrictEqual({
			dogCount: 29.453,
		});
	});

	it("handles a negative int", () => {
		expect(parseEnv({ dogCount: "-32" }, { dogCount: z.number() })).toStrictEqual({
			dogCount: -32,
		});
	});

	it("handles a negative float", () => {
		expect(parseEnv({ dogCount: "-329.3" }, { dogCount: z.number() })).toStrictEqual({
			dogCount: -329.3,
		});
	});

	it("throws on a number schema given a string with leading dash", () => {
		expect(() => parseEnv({ dogCount: "-323hello3.3942" }, { dogCount: z.number() })).toThrow();
	});
	...
	it("handles an object", () => {
		...
		const x = parseEnv(
			{
				ANIMALS: '{ "dog": { "sound": "woof", "size": "big" }, "cat": { "sound": "meow", "size": "small" } }',
			},
			{
				ANIMALS: z.object({
					dog: animal,
	...
	it("handles an intersection type", () => {
		...
		expect(() =>
			parseEnv({ pet: '{ "sound": "woof", "size": "big" }' }, { pet: z.intersection(animal, thingWithSmell) }),
		).toThrow();
	});
	...
	it("doesn't pass through any env values not in the schema", () => {
		const x = parseEnv({ HOST: "localhost", PORT: "5050" }, { HOST: z.string() });
	...
	it("handles spec-provided defaults for common schema types", () => {
		...
		for (const [schema, defaultValue] of schemasWithDefaults) {
			const { SOME_SCHEMA } = parseEnv({}, { SOME_SCHEMA: { schema, defaults: { _: defaultValue } } as any });
	...
	it("handles validator-provided defaults for common schema types", () => {
		...
		for (const [schema, defaultValue] of schemasWithDefaults) {
			const { SOME_SCHEMA } = parseEnv({}, { SOME_SCHEMA: schema.default(defaultValue) });
	...
	it("throws when a value is passed to z.undefined()", () => {
		expect(() => parseEnv({ DEPRECATED: "something" }, { DEPRECATED: z.undefined() })).toThrow();

		expect(() => parseEnv({ DEPRECATED: "" }, { DEPRECATED: z.undefined() })).toThrow();
	...
	it("handles a schema with refinement type", () => {
		...
		expect(() => parseEnv({ AT_LEAST_FIVE_WORDS: "only four words here" }, schema)).toThrow();

		expect(() => parseEnv({ AT_LEAST_FIVE_WORDS: "but there's five words here!" }, schema)).not.toThrow();
	});
